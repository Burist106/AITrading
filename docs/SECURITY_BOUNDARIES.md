...
Aurum Console is fail-closed and `DEMO ONLY`. Milestone 1 adds a local control-plane database and durable intent queue but remains intentionally incapable of contacting a broker or modifying an order or Position. Static fixtures and database schema foundations may illustrate future states, but they do not grant capability.

## Credential boundary

No real credential is needed for Milestone 1. Do not read, request, print, log, commit, upload, or place into a prompt:
...
`.env.example` contains names and empty values only. A local `.env` is ignored and must never be shared. Future MT5 credentials remain only on the Windows Execution Node.

## Trust zones

| Zone                   | Milestone 1 authority                                                                                | Explicitly prohibited                                                                                                 |
| ---------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Browser / web app      | Render validated fixtures and read owner-authorized control-plane state                              | Broker access; direct operational DML; risk-result mutation; secrets; order/Position mutation                         |
...
| Python Worker scaffold | Validate Pydantic models and exercise fake ports                                                     | Polling operational commands; MetaTrader runtime import; broker writes; real credentials; strategy or execution logic |
| Dedicated Worker role  | Fake local claims may execute claim/lease/lifecycle, heartbeat, and incident functions for one owner | Browser use; unrelated-owner access; direct table DML; broker/order/execution/Position writes                         |
| Local Supabase         | Enforce schema, forced RLS, secured functions, queue, audit, and deterministic seed                  | Remote linking/push; production credentials; external calls; broad browser grants                                     |
...
8. Realtime is disabled. If introduced later as a wake-up mechanism, the committed database row remains command truth.
...
- Milestone 2: read-only MT5 adapter with Demo verification and Live fail-closed behavior.
