...
| `broker_symbols`                | Immutable confirmed broker-symbol binding    | Owner/account binding, canonical `XAUUSD`, base `XAU`, profit `USD`, confirmed fingerprint/status/actor/time, and positive confirmation version                       |
...
| `mt5_account_observations`      | Sanitized account verification evidence      | Masked/fingerprinted identity only; append-oriented; no raw login, name, or server                                                                                    |
| `mt5_symbol_observations`       | Versioned normalized XAUUSD specification    | Complete Decimal-string payload and stable fingerprint; append-only evidence that never confirms itself                                                               |
| `mt5_latest_tick_observations`  | One current tick per owner/account/symbol    | Worker-only versioned upsert; bounded current state rather than raw tick history; no per-tick security-audit append                                                   |
| `mt5_reconciliation_runs`       | Full safety-comparison lifecycle             | Idempotent start/complete, confirmed-binding comparison, safe counts/result codes, no operational mutation                                                            |
| `mt5_reconciliation_mismatches` | Append-only uncertainty evidence             | Safe category/reference/severity with unresolved state reserved for a later workflow                                                                                  |
| `mt5_history_query_evidence`    | Per-run bounded Order/Deal query evidence    | Exactly one owner-bound row for each history kind, exact request/result boundaries and state, append-only                                                             |
...
erDiagram
    ...
    MT5_RECONCILIATION_RUNS ||--o{ MT5_HISTORY_QUERY_EVIDENCE : proves
```

Observed symbol rows and confirmed binding rows are deliberately different authorities. A newly recorded fingerprint cannot become its own baseline. The seeded local fixture is fictional and explicitly confirmed by a setup actor; production browser and Worker confirmation functions are absent. Full reconciliation completion atomically records exactly one Order and one Deal query evidence row and requires its exact normalized mismatch payload to equal the persisted child set under the owner-scoped parent lock. Completed runs reject new mismatch evidence. A valid empty tuple is represented as an empty successful result, while a native failure, incomplete window, or unknown window remains blocking evidence.
...
`pnpm db:check` performs two clean resets, runs schema linting, executes nine pgTAP suites, runs the credential-free two-session concurrent-claim integration, and compares a freshly generated type file with `packages/contracts/src/database.generated.ts`. The MT5 suites prove forced RLS, owner isolation, exact Worker RPC authority, canonical XAU/USD validation, immutable confirmed binding, current per-run Order/Deal evidence, bounded tick and component-heartbeat upserts without routine audit growth, idempotent reconciliation, and absence of direct browser/Worker writes.
