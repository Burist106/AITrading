...
These local migrations establish the Milestone 1 control plane and the Milestone 2 sanitized MT5 observation foundation. `broker_orders`, `trade_executions`, and `positions` remain protected read-model foundations only. No database action creates an order, records a simulated execution, changes a Position, or calls an external system.
...
| `mt5_symbol_observations`       | Versioned normalized XAUUSD specification      | Complete Decimal-string payload and stable fingerprint; append-only                                                                                                   |
| `mt5_latest_tick_observations`  | One current tick per owner/account/symbol      | Worker-only versioned upsert; bounded current state rather than raw tick history                                                                                      |
| `mt5_reconciliation_runs`       | Restart/reconnect comparison lifecycle         | Idempotent start/complete, safe counts and result codes, no operational mutation                                                                                      |
...
`pnpm db:check` performs two clean resets, runs schema linting, executes seven pgTAP suites, runs the credential-free two-session concurrent-claim integration, and compares a freshly generated type file with `packages/contracts/src/database.generated.ts`. The seventh suite proves forced RLS, owner isolation, exact Worker RPC authority, decimal-string validation, bounded tick upsert, idempotent reconciliation, and absence of direct browser/Worker writes.
