...
`pnpm db:check` performs two clean resets, runs schema linting, executes eight pgTAP suites, runs the credential-free two-session concurrent-claim integration, and compares a freshly generated type file with `packages/contracts/src/database.generated.ts`. The MT5 suites prove forced RLS, owner isolation, exact Worker RPC authority, canonical XAU/USD validation, immutable confirmed binding, current per-run Order/Deal evidence, bounded tick upsert without audit growth, idempotent reconciliation, and absence of direct browser/Worker writes.
